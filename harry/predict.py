		...
		while True:
			...
			if self.repetition_penalty > 0.:
				for i, token_id in enumerate(reversed(history)):
					if self.vocab.itos[token_id] == "▁br": continue
...
def main():
	...
	while True:
		...
		if text.startswith("/n "): ## Pituus
			models.n = int(text.split(" ")[1])
			models.end = None
		elif text.startswith("/temp "): ## Skaalaa jakaumaa, < 1 lisää varmuutta, > 1 antaa satunnaisempaa
			models.temperature = float(text.split(" ")[1])
		elif text.startswith("/repe "): ## Asettaa toistorangaistuksen, 0 ei rangaistusta, 1 max
			models.repetition_penalty = float(text.split(" ")[1])
		elif text.startswith("/end "): ## Asettaa tokenin minkä generointi päättää novellin
			models.end = text.split(" ")[1].replace("_", "▁")
			models.n = None
		elif text.startswith("/allow "): ## Asettaa regexin jonka perusteella päätetään sallitut merkit RIKKI
			models.allowed = re.compile(text[text.index(" ")+1:].replace("_", "▁"))
		elif text.startswith("/promote "): ## Kymmenkertaistaa annetujen tokenien todennäköisyyden
			models.promoted_tokens.append(text.split(" ")[1].replace("_", "▁"))
		else:
